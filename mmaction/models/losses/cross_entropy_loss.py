...
class KLLoss(nn.Module):
    ...
    def _forward(self, preds_S: torch.Tensor, preds_T: torch.Tensor,
                 **kwargs) -> torch.Tensor:
        """Forward computation.
        ...
        """
        assert preds_S.dim() == 2, 'Only support 2-dim soft label'
        assert preds_T.dim() == 2, 'Only support 2-dim soft label'
        assert len(kwargs) == 0, \
            ('For now, no extra args are supported for soft label, '
                f'but get {kwargs}')
